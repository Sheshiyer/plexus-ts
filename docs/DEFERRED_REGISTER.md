...
| signed OTA upgrade for current candidate | Release Candidate plus protected Publish OTA workflows | Required before full production-ready claim | `npm run verify:all` is local deterministic proof; it cannot prove signed artifacts or the public update feed. | Tag, secret-free candidate pass, protected publication pass, public `latest-mac.yml`, asset checksums, and true upgrade from a prior signed version. |
| automatic update consent prompt | OTA-AUTO-PROMPT-LIVE-1 | Required before claiming automatic discovery is live | The installed `0.5.2` app has no startup scheduler and the public manifest still advertises `0.5.2`, so no current binary can surface a newer-version prompt yet. | After the protected `0.5.3` publication, use one manual `0.5.2` Check to bridge versions; then launch signed `0.5.3` against a newer signed manifest, capture the global download prompt, choose Later, download, confirm the restart prompt, and verify the relaunched version. |
