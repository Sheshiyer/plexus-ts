...
Reviewed: 2026-09-05. Source contract: `package.json`, `src/main/updates.ts`, and `.github/workflows/publish-ota.yml`. Current acceptance: [ISA.md](../ISA.md); documentation ownership: [DOCUMENTATION_MAP.md](DOCUMENTATION_MAP.md).
...
The historical July closeout packet lives at `docs/evidence/2026-07-10-release-candidate-closeout/README.md`. It does not replace the signed OTA proof required below.
...
For an explicitly authorized local signing run with credentials supplied securely, require notarization and verify the resulting artifact as described in [APPLE_SIGNING.md](APPLE_SIGNING.md):

```bash
REQUIRE_NOTARIZATION=true npm run release:mac
```

Only after the migration publication procedure, credential custody, exact candidate, and protected release are authorized: push the reviewed candidate branch and merge through a PR. Never push the candidate directly to `main`. In a clean dedicated release checkout, create the matching tag from the verified merged-main commit; do not switch or clean a dirty user checkout:
...
The required repository policy is `ota-production` with `Sheshiyer` as the required reviewer and a `main`-only deployment policy. The `Protect OTA v* tags` ruleset must restrict creation, update, and deletion of matching tags to the founder account. `Protect main integration` must require a PR plus successful macOS, Ubuntu, and Windows CI, and block deletion/force-push of `main`. Verify these settings for the release; historical configuration receipts are not a fresh settings audit. Before the next production tag, complete the custody migration by entering all nine uniquely named environment secrets from an approved secure source and confirm every name is present:
...
Keep the signed `v0.7.1` GitHub Release and R2 artifacts as the documented
conservative rollback baseline. Select and verify an additional immediate
canary baseline for each new candidate; do not infer its acceptance from this
...
## Historical version-specific gates

<details>
<summary>June–July 2026 release gates — retained history, not current release instructions</summary>

The following version numbers, helper surfaces, and commands describe earlier candidates. Do not recreate tags, republish old immutable keys, install Paperclip, or reuse old acceptance claims from this appendix. The version-independent controls and Labs bridge sequence above govern new work.

## v0.7.4 → v0.7.5 Upgrade Preparation

The July 27 upgrade-canary packet was
...
- Publish `0.3.0` to the then-current R2 feed at `https://plexus-upgrade.thoughtseed.space/plexus`.
