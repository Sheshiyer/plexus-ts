...
export default function AdminDiagnosticsPanel({ overview }: { overview: AdminDemoOverview | null }) {
  ...
  const assistantRows = useMemo<DiagnosticRow[]>(() => {
    ...
    return [
      ...
      { label: 'configured providers', value: model?.configuredProviders.length ? model.configuredProviders.join(', ') : 'none', detail: 'governed gateway metadata only; credentials never reach diagnostics', tone: model?.configuredProviders.length ? 'accent' : 'idle' },
      { label: 'selected lane', value: textOrDash(model?.laneId), detail: model?.gatewayState, tone: model?.gatewayState === 'ready' ? 'accent' : 'warning' },
